...
> commit **`2834ac8`**（父 `c8e911e`）已推 main（`c8e911e..2834ac8`，远端核对 `2834ac8b39542c47d231663de9520b9d543fbec7`）；**未部署**——测试环境需手动触发 Jenkins build（push 不触发），正式随 v0.3 系列发版。
...
- [ ] 正式环境随 v0.3 系列发版一并上线（`2834ac8` + `00129a0` 同批，待运维 build）
...
**提交与验证**：`00129a0`（父 `6d90623`）已推 main，1 文件 / 8 增 1 删，`npx vue-tsc -b` 通过；**未部署**，与 `2834ac8` 同一次 build 一起验。
