...
> 状态：**✅ 已闭环——运维执行迁移后我独立复验通过（2026-09-18）**；唯一未决=运维是否补了 `migrate resolve` 记录（见 §八）
...
- audienceHint 改动仍滞留工作区未提交，与本次无关
- 闭环信号：运维执行后 `GET /api/exams` 返回 200

## 七、当前进度（09-18 晚，重启接续看这里）
...
- **唯一未决（不影响运行）**：运维是走 `migrate deploy`（账本自动补第 8 行）还是手工 ALTER + `migrate resolve --applied`。一句 SQL 定论：`SELECT COUNT(*), MAX(migration_name) FROM _prisma_migrations;` → 期望 8 / `20260907000000_add_exam_audience`（该行 `applied_steps_count=0` 属正常）。**没补则下次发版前必须补**，否则 P3009 把后续所有迁移卡死
- 发送时务必**附改写版 04 文件**（原 `USE lab_exam;` 版已废弃，勿发勿跑）——本次已随通知附出
...
- 遗留不阻塞：fix-school-admin-password.sql 未执行（沈春洋密码，另定时间）；提高并发三件事待拍板（见 [[lab-exam-2026-09-17-capacity-concurrency-assessment]]）
