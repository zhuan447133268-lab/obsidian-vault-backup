...
## 修复方案（已生成；2026-09-02 运维已执行，回归通过）
...
- [x] 运维在正式库**备份后**执行 `03_fix_questions_prod.sql`（或管理端重新上传 8 份 xlsx 等效）。—— **2026-09-02 已执行**（10:06 生成版；**该 SQL 已完成，绝不可重跑**）
- [x] 执行后我回归：100029018 班级生成试卷不再报 1401；单选题题干显示恢复正常；抽查判断题题干正确。—— **✅ 2026-09-02 回归通过**：判断题 754 道题干全唯一（`badContent=0`）、教师端对 23电子一班「测试考试」组卷 **39/39 成功 `failedCount:0`**、正式与测试为同一构建。证据见 [[session-handoff-2026-09-02-v4-fixes-prod-regression-passed]]
